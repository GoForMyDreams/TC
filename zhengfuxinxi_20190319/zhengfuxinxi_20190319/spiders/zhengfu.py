...
class ZhengfuSpider(scrapy.Spider):
    ...
    def parse_content(self, response):
        ...
        info = ''.join(response.xpath("//div[@class='fl w770']/div[@class='d_ldjj']//text()").extract()).strip().replace('\t','').replace('\n','').replace('\r','')
        maker = response.xpath("//div[@class='fl w770']/div[@class='detail_xq w770']/ul/li[8]/span/text()").extract()[0].replace('\xa0','')
